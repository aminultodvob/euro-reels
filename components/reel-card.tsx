    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        ...
        <div className="mt-auto flex gap-2 pt-1">
          <Button id={`watch-${reel.id}`} size="sm" className="flex-1" onClick={handleOpen}>
            {isPost ? (
              <FileText className="mr-1.5 h-3 w-3" />
            ) : (
              <Play className="mr-1.5 h-3 w-3 fill-current" />
            )}{" "}
            {isPost ? "Open Post" : "Watch Feed"}
          </Button>
          <Button id={`fb-link-${reel.id}`} size="sm" variant="outline" className="flex-1" asChild>
